-- ============================================
-- ULTIMATE ONE SHOT FIX V2 - CORRECTION FINALE
-- ============================================
-- Corrige l'erreur de DEFAULT avec product_status
-- et anticipe TOUTES les autres erreurs possibles

-- ==========================================
-- ÉTAPE 0: NETTOYAGE ET CRÉATION DES TYPES
-- ==========================================

-- Supprimer et recréer le type product_status
DROP TYPE IF EXISTS product_status CASCADE;
CREATE TYPE product_status AS ENUM ('pending_payment', 'pending', 'approved', 'rejected');

-- Supprimer et recréer le type order_status
...
-- Supprimer TOUTES les fonctions existantes avec toutes les signatures possibles
...
-- ===== FIX CRITIQUE: Gérer le changement de type de la colonne status =====
DO $$
BEGIN
  -- Vérifier si la colonne status existe et est de type TEXT
  IF EXISTS (
    ...
    -- ÉTAPE 2: Convertir le type de la colonne
    ALTER TABLE products ALTER COLUMN status TYPE product_status USING status::product_status;
    
    -- ÉTAPE 3: Remettre la valeur par défaut avec le bon type
  ...
  ELSIF EXISTS (
    ...
    -- La colonne existe mais n'est pas de type TEXT
    -- S'assurer qu'elle a la bonne valeur par défaut
    ALTER TABLE products ALTER COLUMN status DROP DEFAULT;
    ALTER TABLE products ALTER COLUMN status SET DEFAULT 'pending_payment'::product_status;
  ELSE
    -- La colonne n'existe pas, la créer avec le bon type
    ALTER TABLE products ADD COLUMN status product_status DEFAULT 'pending_payment'::product_status;
  END IF;
END $$;

-- Même traitement pour orders.status
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.columns 
...
BEGIN
  ...
  RAISE NOTICE '';
  RAISE NOTICE '✅ ============================================';
  RAISE NOTICE '✅ ULTIMATE ONE SHOT FIX V2 - TERMINÉ ! 🎉';
  RAISE NOTICE '✅ ============================================';
