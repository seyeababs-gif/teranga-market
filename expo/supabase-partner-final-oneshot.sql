-- ============================================
-- SYSTÈME PARTENAIRES - ONE-SHOT FIX
-- Corrige tout en anticipant les erreurs
-- ============================================

-- ======================================
-- 1. SUPPRESSION DE TOUTES LES FONCTIONS EXISTANTES
-- ======================================
...
-- ======================================
-- 2. RECRÉATION DES FONCTIONS
-- ======================================

-- Fonction: Récupérer les clients référés par un partenaire
...
-- Fonction: Récupérer les statistiques d'un partenaire
...
-- Fonction: Récupérer les partenaires actifs
...
BEGIN
  ...
  SELECT 
    ...
    COALESCE((
      SELECT COUNT(*)
      FROM products p
      WHERE p.partner_id::uuid = u.id::uuid
         OR (p.discount_code IS NOT NULL 
             AND p.discount_code_applied = true
             AND EXISTS (
               SELECT 1 FROM discount_codes dc 
               WHERE dc.id::text = p.discount_code 
               AND dc.partner_user_id = u.id::uuid
             ))
    ), 0) as total_sales,
    u.created_at
  FROM users u
...
-- ======================================
-- 3. TRIGGER POUR CRÉER LES COMMISSIONS
-- ======================================

-- Fonction trigger: Créer automatiquement une commission quand un produit est approuvé
...
BEGIN
  IF NEW.status = 'approved' AND (OLD.status IS NULL OR OLD.status != 'approved') THEN
    ...
    IF NEW.discount_code IS NOT NULL AND NEW.discount_code_applied = true THEN
      ...
      WHERE dc.id::text = NEW.discount_code;
...
-- FIN DU SCRIPT
-- ======================================
-- Ce script corrige complètement le système en one-shot
-- Les partenaires verront maintenant:
