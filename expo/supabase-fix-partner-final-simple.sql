-- ============================================
-- CORRECTION COMPLÈTE ET SIMPLIFIÉE DU SYSTÈME PARTENAIRE
-- ============================================
...
BEGIN
  ...
  IF v_partner_id IS NULL THEN
    RAISE NOTICE 'Utilisateur avec le téléphone 77651104669 introuvable';
    ...
    RAISE NOTICE 'Utilisateur créé avec ID: %', v_partner_id;
  ELSE
...
DECLARE
  ...
  v_commission_amount NUMERIC;
BEGIN
  SELECT id INTO v_partner_id FROM users WHERE phone = '77651104669' AND is_partner = true;
  
  IF v_partner_id IS NULL THEN
    RAISE EXCEPTION 'Partenaire 77651104669 non trouvé';
  END IF;
  
  SELECT COALESCE(partner_commission_rate, 10) INTO v_commission_rate FROM global_settings LIMIT 1;
  ...
  VALUES (
    gen_random_uuid(),
    'Produit Test Commission ' || to_char(NOW(), 'DD/MM HH24:MI'),
    'Test pour vérifier les commissions',
    50000,
    'Électronique',
...
BEGIN
  ...
  RAISE NOTICE 'Stats - Clients: %, Ventes: %, Commission: % FCFA', 
